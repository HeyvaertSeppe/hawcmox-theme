...
function setCardHtml(el, html) {
  if (!el) return;
  if (el.getAttribute('data-hawc-card') === 'done') return;
  el.innerHTML = html;
...
function replaceSupportPanels(root) {
  if (!root || !root.querySelectorAll) return;
  var selectors = [
    '[id^="pveDcSupport-"] .x-autocontainer-innerCt',
    '[id^="pveDcSupport-"] .x-panel-body',
  ...
  for (var i = 0; i < seen.length; i++) {
    var el = seen[i];
    var txt = (el.textContent || '').replace(/\s+/g, ' ').trim();
    var idText = ((el.id || '') + ' ' + ((el.closest && el.closest('[id^="pveDcSupport-"]')) ? 'support' : '')).toLowerCase();
    if (/no valid subscription/i.test(txt) && /valid subscription for this server/i.test(txt) || idText.indexOf('support') !== -1 && /no valid subscription/i.test(txt)) {
      setCardHtml(el, '' +
        '<div class="hawc-subscription-card">' +
...
function replaceRepositoryPanels(root) {
  if (!root || !root.querySelectorAll) return;
  var found = root.querySelectorAll('.x-panel-body, .x-autocontainer-innerCt, .x-container, .x-component');
  for (var i = 0; i < found.length; i++) {
    var el = found[i];
    if (el.querySelector && el.querySelector('.hawc-repo-card')) continue;
    var txt = (el.textContent || '').replace(/\s+/g, ' ').trim();
    if (/The enterprise repository is enabled, but there is no active subscription!?/i.test(txt)) {
      setCardHtml(el, '' +
...
function fixViewGearButton() {
  ...
  var btn = parent.querySelector('.x-btn');
  if (!btn) return;
  var wrappers = [view, parent, parent.parentNode, parent.parentNode ? parent.parentNode.parentNode : null, document.getElementById('container-1039'), document.getElementById('container-1039-innerCt'), document.getElementById('container-1039-targetEl')];
  for (var i = 0; i < wrappers.length; i++) {
    if (!wrappers[i]) continue;
    wrappers[i].style.setProperty('overflow', 'visible', 'important');
  }
  if (document.getElementById('container-1039')) document.getElementById('container-1039').style.setProperty('width', '276px', 'important');
  if (document.getElementById('container-1039-innerCt')) document.getElementById('container-1039-innerCt').style.setProperty('width', '276px', 'important');
  if (document.getElementById('container-1039-targetEl')) document.getElementById('container-1039-targetEl').style.setProperty('width', '276px', 'important');
  ...
  var icon = btn.querySelector('.fa-gear');
  if (icon) {
