...
function renameSubscriptionEntries(root) {
  ...
  var items = root.querySelectorAll('.x-treelist-item-text, .x-tab-inner, .x-menu-item-text, .x-btn-inner, .x-panel-header-title, .x-title-text');
...
function removeSubscriptionWarnings(root) {
  if (!root || !root.querySelectorAll) return;
  var nodes = root.querySelectorAll('.x-panel, .x-panel-body, .x-window, .x-message-box, .x-component, .x-box-item, .x-grid-item');
  for (var i = 0; i < nodes.length; i++) {
    var el = nodes[i];
    if (el.querySelector && el.querySelector('.hawc-subscription-card')) continue;
    if (el.classList && el.classList.contains('hawc-repo-ok')) continue;
    var txt = (el.textContent || '').replace(/\s+/g, ' ').trim();
    if (/There is no subscription key/i.test(txt) || (/No valid subscription/i.test(txt) && !/You do not have a valid subscription for this server/i.test(txt))) {
      if (txt.length < 260) {
...
function fixViewGearButton() {
  ...
  if (!btn) return;
  var outer = parent.parentNode;
  if (outer) {
    outer.style.setProperty('overflow', 'visible', 'important');
    outer.style.setProperty('width', '248px', 'important');
  }
  if (outer && outer.parentNode) {
    outer.parentNode.style.setProperty('overflow', 'visible', 'important');
    outer.parentNode.style.setProperty('width', '248px', 'important');
  }
  parent.style.setProperty('overflow', 'visible', 'important');
  parent.style.setProperty('width', '248px', 'important');
  view.style.setProperty('width', '216px', 'important');
  view.style.setProperty('min-width', '216px', 'important');
  view.style.setProperty('max-width', '216px', 'important');
  btn.style.setProperty('left', '220px', 'important');
