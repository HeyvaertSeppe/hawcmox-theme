...
# HAWCMOX Proxmox UI Manager v5.6 - For Proxmox 8+ and 9.2.2+
# Unified script to Install or Remove the custom theme, repositories, and remove nags.
...
/* Make sure the gear icon right after it doesn't get pushed out or glitched */
#view ~ .x-btn,
.x-btn:has(.fa-gear) {
...
EOF_CSS
    ...
    printf '[4/8] Configuring Proxmox No-Subscription Repository...\n'
    ...
    printf '[5/8] Generating UI patcher script...\n'
    cat > "$JS_FILE" <<'EOF_JS'
(function () {
    ...
    function replaceRepositoryPanels(root) {
      ...
      for (var i = 0; i < nodes.length; i++) {
        var el = nodes[i];
        var txt = (el.textContent || '').replace(/\s+/g, ' ').trim();
        if (/The enterprise repository is enabled, but there is no active subscription!?/i.test(txt) || /No-subscription repository mode active/i.test(txt)) {
          setCardHtml(el, '' +
            '<div class="hawc-repo-card">' +
              '<div class="hawc-repo-check fa fa-check-circle"></div>' +
              '<div class="hawc-card-title">Repository OK</div>' +
              '<div class="hawc-card-text">HAWCMOX no-subscription repository active.</div>' +
            '</div>');
        }
      }
    }
    ...
    function processRoot(root) {
        walkText(root);
        tagAllButtons(root);
        renameSubscriptionEntries(root);
        replaceSupportPanels(root);
        replaceRepositoryPanels(root);
...
EOF_JS
    ...
    printf '[6/8] Creating persistent patcher...\n'
