...
# HAWCMOX Proxmox UI Manager v7.2 - For Proxmox 8+ and 9.2.2+
...
/* --- EXACT FIX: Push down Left Navigation without fighting ExtJS layout --- */
...
(function () {
    ...
    function updateUI() {
        ...
        // 5. Replace "Documentation" header button safely (PREVENTS TASKS CRASH)
        ...
        for (var s = 0; s < docSpans.length; s++) {
            if (docSpans[s].textContent.trim() === 'Documentation') {
                ...
                if (docBtn && docBtn.getAttribute('data-hawc-support') !== 'done') {
                    ...
                    // We modify the existing button instead of adding a new HTML node.
                    // This perfectly preserves the ExtJS index math and fixes the Tasks tab crash!
                    ...
                    // Force the button to be visible, as Proxmox hides it on some screens
                    docBtn.style.setProperty('display', 'inline-block', 'important');
                    
                    // Intercept the click to stop the Proxmox Help Window and open Support instead
