...
# HAWCMOX Proxmox UI Manager v6.8 - For Proxmox 8+ and 9.2.2+
...
/* --- Popups, Tasks Window, and Message Boxes --- */
.x-window, .x-window-default, .x-message-box {
    background: var(--hawc-panel) !important;
...
iframe, .x-html-editor-wrap iframe, .x-panel-body[style*="background-color: white"] {
    ...
    color: var(--hawc-text) !important;
    color-scheme: dark !important; /* Makes native scrollbars dark */
}
/* ------------------------------------------------ */
...
/* --- LOGO AND MENU SPACING (SAFE METHOD) --- */
...
/* Push the left menu content down to make room for the logo safely */
        ...
        for (var s = 0; s < docSpans.length; s++) {
            if (docSpans[s].textContent.trim() === 'Documentation') {
                ...
                if (docBtn && docBtn.getAttribute('data-hawc-support') !== 'done') {
                    ...
                    // ADDED: Perfect 10px gap matching Create VM/CT
