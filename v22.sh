...
# HAWCMOX Proxmox UI Manager v5.7 - For Proxmox 8+ and 9.2.2+
...
/* --- Repository Warning -> Success Styling for inline rows --- */
.hawc-repo-ok,
...
/* --- Flexbox override to snap Server View and Gear Icon together properly --- */
.x-toolbar:has(#view) .x-box-target {
    display: flex !important;
    align-items: center !important;
    gap: 6px !important;
}
...
#view {
    width: 220px !important; /* Force a consistent width that ExtJS understands */
}

/* Ensure the gear icon displays cleanly next to it */
...
(function () {
    ...
    function removeSubscriptionWarnings(root) {
      ...
      for (var i = 0; i < nodes.length; i++) {
        var el = nodes[i];
        if (el.querySelector && (el.querySelector('.hawc-subscription-card') || el.classList.contains('hawc-repo-ok'))) continue;
    ...
    // Safely transforms the repository warnings in place, leaving the rest of the grid intact
        ...
        if (/The enterprise repository is enabled, but there is no active subscription!?/i.test(txt)) {
          ...
          while ((n = walker.nextNode())) {
            ...
            if (/^The enterprise repository is enabled, but there is no active subscription!?$/i.test(clean)) {
              n.data = 'HAWCMOX no-subscription repository active';
    ...
    function processRoot(root) {
        ...
        normalizeRepositoryWarning(root);
