...
html, body, .x-viewport, .x-body { background: var(--hawc-bg) !important; }
...
.x-panel-header, .x-window-header, .x-panel-header-default, .x-window-header-default {
...
.x-toolbar { background: var(--hawc-panel-alt) !important; border: none !important; }
...
.x-toolbar-default-docked-top .x-box-inner { height: auto !important; overflow: visible !important; }
.x-panel:has(> .x-docked-top) { overflow: visible !important; }

.x-plain-body:has(img[src*="proxmox_logo"]) { background: var(--hawc-bg) !important; overflow: visible !important; }
.x-box-inner:has(img[src*="proxmox_logo"]),
.x-box-target:has(img[src*="proxmox_logo"]),
a:has(img[src*="proxmox_logo"]) { overflow: visible !important; }
...
.x-btn-default-small.x-btn-over, .x-btn-default-medium.x-btn-over {
  background: var(--hawc-accent) !important;
  border-color: var(--hawc-accent) !important;
}
.x-btn[data-hawc-action="reboot"] { background: var(--hawc-warn) !important; border-color: var(--hawc-warn) !important; color: var(--hawc-warn-text) !important; }
.x-btn[data-hawc-action="reboot"] .x-btn-inner { color: var(--hawc-warn-text) !important; }
.x-btn[data-hawc-action="shutdown"], .x-btn[data-hawc-action="stop"] {
  background: var(--hawc-danger) !important; border-color: var(--hawc-danger) !important; color: var(--hawc-danger-text) !important;
}
.x-btn[data-hawc-action="shutdown"] .x-btn-inner, .x-btn[data-hawc-action="stop"] .x-btn-inner { color: var(--hawc-danger-text) !important; }
.x-btn[data-hawc-action="shell"], .x-btn[data-hawc-action="console"] {
  background: var(--hawc-dark-btn) !important; border-color: var(--hawc-dark-btn) !important; color: var(--hawc-dark-text) !important;
}
.x-btn[data-hawc-action="shell"] .x-btn-inner, .x-btn[data-hawc-action="console"] .x-btn-inner { color: var(--hawc-dark-text) !important; }
.x-btn[data-hawc-action="create-vm"], .x-btn[data-hawc-action="create-ct"] {
  background: var(--hawc-create) !important; border-color: var(--hawc-create-border) !important; color: var(--hawc-create-text) !important;
}
...
.x-btn[data-hawc-action="remove-sub"] { opacity: 0.4 !important; pointer-events: none !important; filter: grayscale(100%) !important; }
...
.x-form-trigger-wrap-focus .x-form-text, .x-form-text-focus { border-color: var(--hawc-accent) !important; }

/* --- Tabs --- */
.x-tab { border-radius: var(--hawc-radius-sm) var(--hawc-radius-sm) 0 0 !important; background: var(--hawc-panel) !important; color: var(--hawc-text-dim) !important; }
.x-tab-active { background: var(--hawc-panel-alt) !important; color: var(--hawc-text) !important; }

/* --- Grids / trees --- */
.x-grid-header-ct, .x-column-header { background: var(--hawc-panel-alt) !important; color: var(--hawc-text) !important; border-color: var(--hawc-border) !important; }
.x-grid-row:hover .x-grid-cell, .x-grid-row-over .x-grid-cell { background: var(--hawc-panel-alt) !important; }
.x-grid-row-selected .x-grid-cell { background: rgba(255, 255, 255, 0.08) !important; }
...
.x-boundlist-item-over { background: var(--hawc-accent) !important; }
...
/* --- Forced Sidebar / Resource Tree Layout ---
   NOTE: ExtJS assigns numeric IDs (e.g. pveResourceTree-1011, container-1038)
   dynamically on every reload, so we anchor on stable structural markers
   (role="treegrid" and the static #view combo id) instead of the numeric IDs. */

/* Sidebar wrapper (search box + view selector + tree): force top offset 39px -> 69px */
.x-container:has(#view):has([role="treegrid"]) {
  top: 69px !important;
}

/* Resource tree panel: force height 633px -> 600px */
.x-tree-panel.x-grid[role="treegrid"] {
  height: 600px !important;
}
.x-tree-panel.x-grid[role="treegrid"] .x-panel-body.x-grid-body {
  height: 598px !important;
}
.x-tree-panel.x-grid[role="treegrid"] .x-tree-view.x-fit-item {
...
img[src*="proxmox_logo"] {
  ...
  opacity: 1 !important;
  /* Break out of ExtJS layout and sit above everything, including the header */
...
(function () {
  ...
  function processTextNode(node) {
    ...
    if (changed) { node.data = data.trim(); }
  }
  ...
  function buttonText(btn) {
    ...
    if (!t) { t = btn.getAttribute('data-qtip') || btn.getAttribute('title') || btn.getAttribute('aria-label') || ''; }
    return (t || '').trim();
  }
  ...
  function tagAllButtons(root) {
    if (!root) return;
    if (root.nodeType === 1 && root.classList && root.classList.contains('x-btn')) { tagButton(root); }
    ...
    var observer = new MutationObserver(function (mutations) {
      for (var i = 0; i < mutations.length; i++) {
        ...
        if (m.type === 'characterData') { processTextNode(m.target); }
          ...
          for (var j = 0; j < m.addedNodes.length; j++) { processRoot(m.addedNodes[j]); }
        }
      }
    });
...
if [ -f "$TEMPLATE_FILE" ]; then
  ...
  if ! grep -q "css/hawcmox.css" "$TEMPLATE_FILE"; then
    sed -i "s|</head>|<link rel=\"stylesheet\" type=\"text/css\" href=\"/pve2/css/hawcmox.css\"></head>|" "$TEMPLATE_FILE"
  fi
  if ! grep -q "js/hawcmox.js" "$TEMPLATE_FILE"; then
    sed -i "s|</head>|<script type=\"text/javascript\" src=\"/pve2/js/hawcmox.js\"></script></head>|" "$TEMPLATE_FILE"
  fi
fi
...
if [ -f "$SUBLIB_FILE" ]; then
  ...
  if [ "$HIDE_NAG" = "yes" ]; then
    if [ -f "$SUBLIB_BACKUP" ]; then cp -f "$SUBLIB_BACKUP" "$SUBLIB_FILE"; fi
