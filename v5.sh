...
# HAWCMOX Proxmox UI Manager v5 - For Proxmox 9.2.2+
# Unified script to Install or Remove the custom theme.
# v5 changes: taller top toolbar/header (no more logo clipping on
# right/bottom), smaller logo height that fits inside it, and all
# blue accent colors replaced with neutral slate/grey tones.
...
/* Let the toolbar's inner scroller/box size to content instead of a
   fixed pixel height, so the taller logo/header never gets clipped. */
...
/* The panel that directly wraps the top toolbar must not clip its
   content vertically, otherwise a taller toolbar still gets cut off
   by the panel's overflow:hidden (used everywhere else for corners). */
