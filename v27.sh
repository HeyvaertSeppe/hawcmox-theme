...
# HAWCMOX Proxmox UI Manager v6.5 - For Proxmox 8+ and 9.2.2+
# Unified automated script to Install or Remove the custom theme.
#
# v6.5 CHANGES (fixes requested):
#  - Removed ".x-panel:has(> .x-docked-top) { overflow: visible !important; }"
#    This unscoped rule matched ANY panel with a top-docked toolbar, including
#    the Tasks popup grid, breaking its internal clipping/scrolling.
#  - Added "position: relative !important;" to .x-toolbar-default-docked-top
#    so the absolutely-positioned global search field has a proper positioned
#    ancestor instead of escaping the header.
#  - Removed the unscoped "#view { width: 235px !important; }" block and its
#    gear-icon repositioning hack. That id isn't a documented Proxmox element
#    and was a likely source of the layout collision. If you know exactly
#    which element you meant to target, tell me and I'll re-add it scoped.
#  - Support header button now targets #v1:0:18:=pveDcSupport:::::::27
#  - Support button margin now matches the Create VM / Create CT gap.
#  - Added an "Uninstall HAWCMOX Theme" link under "Contact now" in the
#    support card. Clicking it shows a native-style Proxmox confirmation
#    dialog ("Are you sure?"). NOTE: browser JS cannot run root shell
#    commands, so "Yes" shows the exact command to run server-side to fully
#    revert (this calls the same uninstall_theme() logic already in this
#    script) rather than pretending to do it silently from the page.
...
.x-toolbar-default-docked-top {
    /* NOTE: position:relative added so the absolutely positioned
       #pveGlobalSearchField- element below has a real positioned
       ancestor to anchor to, instead of escaping the header. */
...
/* NOTE: the old ".x-panel:has(> .x-docked-top) { overflow: visible !important; }"
   rule was removed here. It matched every ExtJS panel with a top-docked
   toolbar -- including the Tasks window's grid panel -- and forcing
   overflow:visible on that grid broke its clipping/scrolling, which is
   what was breaking the Tasks popup. The rule below is scoped only to
   the logo's plain-body container, which is what actually needed it. */
...
/* Header "Support" button (replaces Documentation) - margin matched
   to the Create VM / Create CT gap (3px) per request. */
#hawc-support-btn {
    margin: 0 3px !important;
...
.hawc-uninstall-link:hover {
  color: var(--hawc-danger) !important;
}

/* --- Native-style confirmation dialog (mimics x-message-box) --- */
.hawc-confirm-mask {
...
.hawc-confirm-footer {
  ...
  gap: 8px !important;
  border-top: 1px solid var(--hawc-border) !important;
}
...
.hawc-confirm-btn:hover {
  opacity: 0.88 !important;
}

.hawc-confirm-btn.hawc-danger {
  background: var(--hawc-danger) !important;
  border-color: var(--hawc-danger) !important;
  color: #ffffff !important;
}
...
/* --- Exact Search Bar Positioning (Centered) --- */
[id^="pveGlobalSearchField-"] {
...
/* --- Exact Version Text Positioning (Right of Search) --- */
#versioninfo {
    position: absolute !important;
...
(function () {
    ...
    var UNINSTALL_CMD = 'hawcmox-patch.sh --uninstall  (or: bash hawcmox-manager.sh, then choose option 2)';
    ...
    // Native-style confirmation dialog, mimicking a Proxmox x-message-box.
    function showConfirmDialog(opts) {
        var mask = document.createElement('div');
        mask.className = 'hawc-confirm-mask';

        var box = document.createElement('div');
        box.className = 'hawc-confirm-box';
        box.innerHTML =
            '<div class="hawc-confirm-header">' + opts.title + '</div>' +
            '<div class="hawc-confirm-body">' + opts.message + '</div>' +
            '<div class="hawc-confirm-footer">' +
              '<button type="button" class="hawc-confirm-btn" data-hawc-confirm="no">No</button>' +
              '<button type="button" class="hawc-confirm-btn hawc-danger" data-hawc-confirm="yes">Yes</button>' +
            '</div>';
        ...
        box.querySelector('[data-hawc-confirm="no"]').addEventListener('click', close);
        box.querySelector('[data-hawc-confirm="yes"]').addEventListener('click', function () {
            close();
    ...
    function showUninstallConfirm() {
        showConfirmDialog({
            title: 'Uninstall HAWCMOX Theme?',
            message:
                'This will remove the HAWCMOX theme, restore the original Proxmox ' +
                'logo, CSS and JS, and revert the interface back to stock Proxmox. ' +
                'Are you sure?' +
                '<span class="hawc-confirm-cmd">Note: a browser page cannot run root ' +
                'commands on the server for security reasons. Confirming below shows ' +
                'you the exact command to run on the node to complete the revert.</span>',
            onConfirm: function () {
                showConfirmDialog({
                    title: 'Run this on the node',
                    message:
                        'SSH into this Proxmox node as root and run:' +
                        '<span class="hawc-confirm-cmd">' + UNINSTALL_CMD + '</span>' +
                        'This restores the original proxmox_logo, removes hawcmox.css / hawcmox.js, ' +
                        'reinstalls pve-manager and proxmox-widget-toolkit from the repo, and ' +
                        'restarts pveproxy.',
                    onConfirm: function () {}
                });
            }
    ...
    function updateUI() {
        ...
        // 1. Version Info Text modification (Targeted and Safe - NO DOM TREE WALKING)
        ...
        // 2. Action Button styling properties (Lightweight, non-destructive attribute assignment)
        ...
        for (var k = 0; k < targets.length; k++) {
            ...
            if (/No valid subscription/i.test(txt) || el.closest('[id^="pveDcSupport-"]')) {
                ...
                '<div class="hawc-subscription-card">' +
                  ...
                  '<button type="button" class="hawc-uninstall-link" data-hawc-uninstall-trigger>Uninstall HAWCMOX Theme</button>' +
                '</div>';
                el.setAttribute('data-hawc-card', 'done');

                var trigger = el.querySelector('[data-hawc-uninstall-trigger]');
                if (trigger) {
                    trigger.addEventListener('click', showUninstallConfirm);
                }
            }
        }
        ...
        for (var s = 0; s < docSpans.length; s++) {
            if (docSpans[s].textContent.trim() === 'Documentation') {
                ...
                if (docBtn && docBtn.getAttribute('data-hawc-support') !== 'done') {
                    ...
                    a.innerHTML = '<span class="x-btn-wrap" style="display:flex; align-items:center; gap:6px; height: 100%;"><span class="x-btn-icon-el fa fa-life-ring" style="color:#ffffff; margin-top:2px;"></span><span class="x-btn-inner" style="color:#ffffff; font-size:12px;">Support</span></span>';
    ...
    // Modern implementation leveraging DOM throttle to prevent UI freezing 
    var throttleTimeout;
