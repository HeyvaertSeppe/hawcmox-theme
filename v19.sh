...
# HAWCMOX Proxmox UI Manager v5.5 - For Proxmox 9.2.2+
...
/* This targets the Node/VM toolbar specifically */
...
.x-toolbar-default-docked-top .x-box-inner {
    height: auto !important;
    overflow: visible !important;
}

.x-panel:has(> .x-docked-top) {
    overflow: visible !important;
}
...
.x-box-inner:has(img[src*="proxmox_logo"]),
.x-box-target:has(img[src*="proxmox_logo"]),
a:has(img[src*="proxmox_logo"]) {
    overflow: visible !important;
}
...
/* Hide the System Report button */
.x-btn[data-hawc-action="system-report"] {
    display: none !important;
}

/* Disable and grey out the Remove Subscription button */
...
[id^="container-"][style*="left: 5px"] > .x-box-inner,
[id^="container-"][style*="left: 5px"] > .x-box-inner > .x-box-target,
.x-container.x-border-item.x-box-item.x-container-default.x-box-layout-ct[style*="left: 5px"] > .x-box-inner,
.x-container.x-border-item.x-box-item.x-container-default.x-box-layout-ct[style*="left: 5px"] > .x-box-inner > .x-box-target {
  width: 264px !important;
  margin-right: 0 !important;
}

[id^="pveResourceTree-"],
.x-panel.x-grid-header-hidden.x-box-item.proxmox-tags-circle.x-panel-default.x-tree-panel.x-tree-arrows.x-grid {
  top: 29px !important;
  width: 264px !important;
  min-width: 264px !important;
  max-width: 264px !important;
...
[id^="pveResourceTree-"] > .x-panel-bodyWrap,
[id^="pveResourceTree-"] > .x-panel-bodyWrap > .x-panel-body,
...
[id^="pveResourceTree-"] .x-grid-cell,
[id^="pveResourceTree-"] .x-grid-cell-first,
[id^="pveResourceTree-"] .x-grid-cell-last,
[id^="pveResourceTree-"] .x-column-header,
[id^="pveResourceTree-"] .x-box-target,
[id^="pveResourceTree-"] .x-box-inner,
...
.x-tree-view .x-grid-row .x-grid-cell,
.x-grid-item-container,
.x-tree-view,
...
.x-grid-row-over .x-grid-cell,
.x-grid-row-selected .x-grid-cell,
.x-tree-view .x-grid-item-selected .x-grid-cell {
...
/* --- Repository warning -> success style --- */
.hawc-repo-ok,
.hawc-repo-ok .x-panel-body,
.hawc-repo-ok .x-autocontainer-innerCt,
.hawc-repo-ok .x-box-inner,
.hawc-repo-ok .x-box-target,
.hawc-repo-ok .x-grid-item-container,
.hawc-repo-ok .x-grid-cell,
.hawc-repo-ok .x-grid-cell-inner {
  background: var(--hawc-success-bg) !important;
  color: #d1fae5 !important;
  border-color: var(--hawc-success) !important;
}

.hawc-repo-ok .fa-warning,
.hawc-repo-ok .fa-exclamation,
.hawc-repo-ok .fa-exclamation-circle {
  display: none !important;
}

.hawc-repo-check {
  display: block !important;
  color: var(--hawc-success) !important;
...
/* --- Server View and gear button fixes --- */
#view,
[id="view"] {
...
.x-btn[id*="button-"] .x-btn-icon-el.fa-gear,
.x-btn[id*="button-"] {
...
/* --- Support contact card and Server View fixes --- */
...
#view + .x-btn .x-btn-wrap,
#view ~ .x-btn .x-btn-wrap,
#view + .x-btn .x-btn-button,
#view ~ .x-btn .x-btn-button,
#view + .x-btn .x-btn-icon-el,
#view ~ .x-btn .x-btn-icon-el {
  overflow: visible !important;
  color: #ffffff !important;
  opacity: 1 !important;
...
#view-parent,
#container-1039,
#container-1039-innerCt,
#container-1039-targetEl {
  overflow: visible !important;
}

/* --- Support/repository cards and Server View sizing --- */
.hawc-info-card,
.hawc-repo-card,
.hawc-subscription-card {
  display: flex !important;
...
.hawc-repo-card {
  background: var(--hawc-success-bg) !important;
  border: 1px solid var(--hawc-success) !important;
  color: #d1fae5 !important;
}
...
(function () {
  ...
  var img = document.querySelector('img[src*="proxmox_logo"]');
  if (!img) return;
  var host = img.closest('a') || img;
  if (host && host.parentNode !== document.body) {
    document.body.appendChild(host);
  }
  var target = host === img ? img : host;
  var tree = document.querySelector('[id^="pveResourceTree-"]');
  var left = 1;
  if (tree) {
    var rect = tree.getBoundingClientRect();
    var logoWidth = img.getBoundingClientRect().width || 140;
    left = Math.round(rect.left + Math.max(0, (rect.width - logoWidth) / 2));
  }
  target.style.setProperty('position', 'fixed', 'important');
  target.style.setProperty('top', '10px', 'important');
...
function normalizeRepositoryWarning(root) {
  if (!root) return;
  var nodes = [];
  if (root.nodeType === 1) nodes.push(root);
  if (root.querySelectorAll) {
    var found = root.querySelectorAll('.x-panel, .x-panel-body, .x-container, .x-box-item, .x-grid-item, .x-component, .x-treelist');
    for (var i = 0; i < found.length; i++) nodes.push(found[i]);
  }
  for (var i = 0; i < nodes.length; i++) {
    var el = nodes[i];
    var txt = (el.textContent || '').replace(/\s+/g, ' ').trim();
    if (/The enterprise repository is enabled, but there is no active subscription!?/i.test(txt)) {
      el.classList.add('hawc-repo-ok');
      var walker = document.createTreeWalker(el, NodeFilter.SHOW_TEXT, null, false);
      var n;
      while ((n = walker.nextNode())) {
        var clean = (n.data || '').replace(/\s+/g, ' ').trim();
        if (/^The enterprise repository is enabled, but there is no active subscription!?$/i.test(clean)) {
          n.data = 'No-subscription repository mode active';
        } else if (/^Warning$/i.test(clean)) {
          n.data = 'OK';
        }
      }
      var icon = el.querySelector('.fa-warning, .fa-exclamation, .fa-exclamation-circle');
      if (icon) {
        icon.className = 'hawc-repo-check fa fa-check-circle';
        icon.style.setProperty('display', 'block', 'important');
      } else if (!el.querySelector('.hawc-repo-check')) {
        var check = document.createElement('div');
        check.className = 'hawc-repo-check fa fa-check-circle';
...
function replaceSubscriptionContent(root) {
  if (!root || !root.querySelectorAll) return;
  var blocks = root.querySelectorAll('.x-panel-body, .x-autocontainer-innerCt, .x-container, .x-component');
  for (var i = 0; i < blocks.length; i++) {
    var el = blocks[i];
    if (el.querySelector && el.querySelector('.hawc-subscription-card')) continue;
    var txt = (el.textContent || '').replace(/\s+/g, ' ').trim();
    if (/No valid subscription/i.test(txt) && /You do not have a valid subscription for this server/i.test(txt)) {
      el.innerHTML = '' +
        '<div class="hawc-subscription-card">' +
          '<div class="hawc-subscription-title">HAWCMOX Support</div>' +
          '<div class="hawc-subscription-text">Contact info: <a class="hawc-subscription-link" href="mailto:support@hawc.be">support@hawc.be</a></div>' +
          '<div class="hawc-subscription-text">Use the button below to open a prefilled HAWCMOX support email.</div>' +
...
function renameSubscriptionEntries(root) {
  if (!root || !root.querySelectorAll) return;
  var items = root.querySelectorAll('.x-treelist-item-text, .x-tab-inner, .x-menu-item-text, .x-btn-inner, .x-panel-header-text, .x-title-text, h1');
  for (var i = 0; i < items.length; i++) {
    var t = (items[i].textContent || '').replace(/\s+/g, ' ').trim();
    if (/^Subscription$/i.test(t)) {
      items[i].textContent = 'Support';
    }
  }
}

function removeSubscriptionWarnings(root) {
  if (!root || !root.querySelectorAll) return;
  var nodes = root.querySelectorAll('.x-window, .x-message-box, .x-panel, .x-panel-body, .x-component, .x-box-item, .x-grid-item');
  for (var i = 0; i < nodes.length; i++) {
    var el = nodes[i];
    if (el.querySelector && (el.querySelector('.hawc-subscription-card') || el.querySelector('.hawc-repo-card'))) continue;
    var txt = (el.textContent || '').replace(/\s+/g, ' ').trim();
    if (/There is no subscription key/i.test(txt)) {
      el.style.setProperty('display', 'none', 'important');
    }
    if (/No valid subscription/i.test(txt) && !/valid subscription for this server/i.test(txt) && txt.length < 300) {
      el.style.setProperty('display', 'none', 'important');
    }
  }
}
...
function replaceSupportPanels(root) {
    ...
    if (/No valid subscription/i.test(txt) || el.closest('[id^="pveDcSupport-"]')) {
      setCardHtml(el, '' +
        '<div class="hawc-subscription-card">' +
          ...
          '<div class="hawc-card-text">Contact info: <a class="hawc-card-link" href="mailto:support@hawc.be">support@hawc.be</a></div>' +
...
function replaceRepositoryPanels(root) {
  if (!root || !document.querySelectorAll) return;
  var nodes = document.querySelectorAll('.x-panel-body, .x-autocontainer-innerCt, .x-component');
  for (var i = 0; i < nodes.length; i++) {
    var el = nodes[i];
    var txt = (el.textContent || '').replace(/\s+/g, ' ').trim();
    if (/The enterprise repository is enabled, but there is no active subscription!?/i.test(txt)) {
      setCardHtml(el, '' +
        '<div class="hawc-repo-card">' +
          '<div class="hawc-repo-check fa fa-check-circle"></div>' +
          '<div class="hawc-card-title">Repository OK</div>' +
          '<div class="hawc-card-text">No-subscription repository mode active.</div>' +
        '</div>');
    }
  }
}

function fixViewGearButton() {
  var c = document.getElementById('container-1039');
  var ci = document.getElementById('container-1039-innerCt');
  var ct = document.getElementById('container-1039-targetEl');
  var view = document.getElementById('view');
  var body = document.getElementById('view-bodyEl');
  var triggerWrap = document.getElementById('view-triggerWrap');
  var inputWrap = document.getElementById('view-inputWrap');
  var input = document.getElementById('view-inputEl');
  var btn = document.getElementById('button-1040') || (view && view.parentNode ? view.parentNode.querySelector('.x-btn') : null);
  var icon = document.getElementById('button-1040-btnIconEl') || (btn ? btn.querySelector('.fa-gear') : null);

  [c, ci, ct, view, body, triggerWrap, inputWrap, input, btn].forEach(function (el) {
    if (el) el.style.setProperty('overflow', 'visible', 'important');
  });

  if (c) c.style.setProperty('width', '276px', 'important');
  if (ci) ci.style.setProperty('width', '276px', 'important');
  if (ct) ct.style.setProperty('width', '276px', 'important');
  if (view) {
    view.style.setProperty('width', '247px', 'important');
    view.style.setProperty('min-width', '247px', 'important');
    view.style.setProperty('max-width', '247px', 'important');
    view.style.setProperty('left', '0px', 'important');
  }
  if (body) body.style.setProperty('width', '142px', 'important');
  if (triggerWrap) triggerWrap.style.setProperty('width', '142px', 'important');
  if (inputWrap) inputWrap.style.setProperty('width', '118px', 'important');
  if (input) input.style.setProperty('width', '118px', 'important');
  if (btn) {
    btn.style.setProperty('left', '252px', 'important');
    ...
    btn.style.setProperty('max-width', '24px', 'important');
